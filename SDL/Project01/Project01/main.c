...
int main(int argc, char** argv)
{
	...
	int height;
	int channels;
	unsigned char* pixels = stbi_load("car.png", &width, &height, &channels, 4);
	if (!pixels)
	{
...
void render_to_target(SDL_Renderer* renderer, SDL_Texture* target, struct nk_context* ctx, void* data, SDL_Vertex* vertices, int* indices, SDL_Texture* texture)
{
	...
	if (nk_begin(ctx, "Window", nk_rect(50, 50, 220, 220), NK_WINDOW_BORDER | NK_WINDOW_MOVABLE | NK_WINDOW_CLOSABLE))
	{
